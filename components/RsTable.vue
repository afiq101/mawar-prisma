...
const fetchDataFromURL = async () => {
  ...
  try {
    ...
    if (data.value.statusCode === 200) {
      dataTable.value = data.value.body.data;
      dataLength.value = data.value.body.totalPage;
...
const computedData = computed(() => {
  ...
  result = dataTable.value
    ...
    .filter((row) => {
      ...
      if (keyword.value === "" || props.url) return true;
      let result = false;
  ...
  if (!props.url) dataLength.value = totalData;
...
// watch currentPage if change then fetch data
watch(
  () => currentPage.value,
  async () => {
    if (props.url) await fetchDataFromURL();
  },
  { deep: true }
);

// watch pageSize if change then fetch data
watch(
  () => pageSize.value,
  async () => {
    if (props.url) await fetchDataFromURL();
...
// watchDebounced keyword if change then fetch data
watchDebounced(
  () => keyword.value,
  async () => {
    if (props.url) await fetchDataFromURL();
...
<template>
  <div>
    <div v-if="loading" class="text-center my-4">
      <Icon name="svg-spinners:270-ring-with-bg" class="mr-4"> </Icon>
      <strong>Loading Data. This may take a while.</strong>
    </div>
    <div
      v-else-if="!loading && dataTable && dataTable.length > 0"
      class="table-wrapper"
      :class="{
        ...
        >
          ...
          >
            <rs-dropdown-item @click="hideColumn(val)">
              ...
              <Icon
                :name="
                  getFilter(val) ? 'mdi:eye-outline' : 'mdi:eye-off-outline'
                "
                size="1rem"
                class="ml-auto"
              ></Icon>
    ...
    <div v-else class="flex items-center justify-center my-4">
      ...
      <div>
        ...
        <p>Check your data or try again later.</p>
      </div>
    </div>
  </div>
</template>
